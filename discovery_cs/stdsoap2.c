...
{ if (soap->logfile[i])
    soap->fdebug[i] = fopen(soap->logfile[i], i < 2 ? "wb" : "a");
}
#endif
...
{ if (!soap_peek_element(soap))
  { if (soap->other)
    ...
    if (tag && *tag == '-')
      return SOAP_OK;
	//printf("soap_element_begin_in %s %s\r\n", soap->tag, tag);
	...
	//else
	//	printf("soap_element_begin_in %s %s failed=%d\r\n", soap->tag, tag,soap->error);
  }
...
{
	//(2011-8-9 duwenyong)modify，在Envelope后不发送"\r\n"两个字符，
	//否则海康DS-2CD863PF(NF)-E(W) 会返回matching rule specified is not supported.
  //if (soap_element_end_out(soap, "SOAP-ENV:Envelope") || soap_send_raw(soap, "\r\n", 2))	/* 2.8: always emit \r\n after SOAP Envelope */
  if (soap_element_end_out(soap, "SOAP-ENV:Envelope"))	/* 2.8: always emit \r\n after SOAP Envelope */
    return soap->error;
#ifndef WITH_LEANER
